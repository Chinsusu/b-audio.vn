...
export const analytics = {
  event(name: string, params?: Record<string, any>) {
    if (typeof window !== 'undefined') {
      ...
      // FB Pixel
      // @ts-ignore
      window.fbq && window.fbq('trackCustom', name, params || {});
    }
  },
  view_item_list(items: any[], list_name: string) {
    this.event('view_item_list', {
      ...
      items: items.map(item => ({
        ...
        item_category: item.attributes?.category?.data?.attributes?.name || 'uncategorized',
  ...
  add_to_quote(product: any) {
    this.event('add_to_quote', {
      ...
      items: [{
        ...
        item_category: product.attributes?.category?.data?.attributes?.name || 'uncategorized',
