...
export const metadata: Metadata = {
  ...
  description:
    "Danh sách sản phẩm loa B‑Audio: portable, karaoke... với hình ảnh và giá cập nhật.",
...
export default async function ProductsPage({
      ...
      itemListElement: [
        {
          ...
          position: 1,
          name: "Trang chủ",
          item: "https://b-audio.vn/",
        },
        {
          "@type": "ListItem",
          position: 2,
          name: "Sản phẩm",
          item: "https://b-audio.vn/products",
        },
      ],
    },
  } as any;
  ...
  const itemList = {
    ...
    itemListElement: products.map((p: any, i: number) => {
      ...
      const rel = attrs.images?.data?.[0]?.attributes?.url as
        | string
        | undefined;
      const imageUrl = rel
        ? /^https?:\/\//.test(rel)
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-12">
      ...
      <form className="mt-6 grid grid-cols-1 md:grid-cols-5 gap-3" method="get">
        ...
        <select
          name="sort"
          defaultValue={searchParams?.sort || "publishedAt:desc"}
          className="rounded border px-3 py-2"
        >
      ...
      {products.length === 0 ? (
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {products.map((p: any) => {
            const attrs = p.attributes || {};
            const img = attrs.images?.data?.[0]?.attributes?.url as
              | string
              | undefined;
            const imageUrl = img
              ? /^https?:\/\//.test(img)
                ...
                : `${API_BASE}${img}`
              : undefined;
            const cat = attrs.category?.data?.attributes?.name as
              | string
              | undefined;
            const catSlug = attrs.category?.data?.attributes?.slug as
              | string
              | undefined;
            return (
              <Link
