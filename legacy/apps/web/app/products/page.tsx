...
export const metadata: Metadata = {
  ...
  description:
    "Danh sách sản phẩm loa B‑Audio: portable, karaoke với hình ảnh, giá cập nhật và bộ lọc thông minh.",
  ...
  openGraph: {
    ...
    images: [
      {
        url: "https://b-audio.vn/og/products-cover.svg",
        width: 1200,
        height: 630,
        alt: "B-Audio Products",
      },
    ],
  },
  twitter: {
    card: "summary_large_image",
    images: ["https://b-audio.vn/og/products-cover.svg"],
  },
};
...
function mapSort(sort?: string): string {
  switch (sort) {
    ...
    case "price-desc":
      return "price:desc";
    case "power-asc":
      return "power_watt:asc";
    case "power-desc":
      return "power_watt:desc";
    case "newest":
  ...
  try {
    ...
    return (
      <main className="mx-auto max-w-6xl px-6 py-12">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-heading text-h1 text-textWhite font-bold">
            Sản phẩm B-Audio
          </h1>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="mb-6 flex items-center justify-between">
              ...
              {searchParams?.search || searchParams?.category ? (
                <Link
                  href="/products"
                  className="font-body text-body text-neonTurquoise hover:underline focus:outline-none focus:ring-2 focus:ring-neonTurquoise focus:ring-offset-2 focus:ring-offset-darkBg rounded"
                >
                  Xóa bộ lọc
                </Link>
              ) : null}
            ...
            {products.length === 0 ? (
              <div className="py-20 text-center text-textWhite">
                <div className="font-heading text-h3 text-textWhite font-semibold mb-2">
                  Không tìm thấy sản phẩm
                </div>
                <p className="font-body text-body text-textGrey mb-4">
            ...
            {pageCount > 1 && (
              <nav className="mt-12 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {prevDisabled ? (
                    <span className="rounded-2xl border border-darkGrey px-4 py-2 text-textGrey/40 cursor-not-allowed">
                      ← Trước
                    </span>
                  ) : (
                    <Link
    ...
    return (
      <main className="mx-auto max-w-3xl px-6 py-16">
        <div className="rounded-2xl border border-darkGrey bg-darkGrey/40 p-8 backdrop-blur-sm">
          <h1 className="font-heading text-h2 text-textWhite font-bold">
            Không tải được danh sách sản phẩm
          </h1>
          <p className="mt-2 font-body text-body text-textGrey">
            Hệ thống đang gặp trục trặc tạm thời. Vui lòng thử lại sau ít phút.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              href="/"
              className="rounded-2xl bg-goldAccent px-4 py-2 text-darkBg shadow-glowGold hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-goldAccent focus:ring-offset-2 focus:ring-offset-darkBg transition-shadow"
            >
              Về trang chủ
            </a>
            <a
              href="/products"
              className="rounded-2xl border border-darkGrey px-4 py-2 text-textWhite hover:bg-darkGrey/60 focus:outline-none focus:ring-2 focus:ring-neonTurquoise focus:ring-offset-2 focus:ring-offset-darkBg transition-colors"
            >
              Tải lại trang
            </a>
          </div>
